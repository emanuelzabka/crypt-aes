...
func TestSubBytes(t *testing.T) {
	...
	for i := range source {
		if state[i] != expected[i] {
			t.Errorf("Invalid subbytes for 0x%s, got: 0x%s, want: 0x%s",
...
func TestInvSubBytes(t *testing.T) {
	source := []byte{0x7a, 0x9f, 0x10, 0x27, 0x89, 0xd5, 0xf5, 0x0b, 0x2b, 0xef, 0xfd, 0x9f, 0x3d, 0xca, 0x4e, 0xa7}
	expected := []byte{0xbd, 0x6e, 0x7c, 0x3d, 0xf2, 0xb5, 0x77, 0x9e, 0x0b, 0x61, 0x21, 0x6e, 0x8b, 0x10, 0xb6, 0x89}
	state := make([]byte, len(source))
	copy(state, source)
	invSubBytes(state)
	for i := range source {
		if state[i] != expected[i] {
			t.Errorf("Invalid invsubbytes for 0x%s, got: 0x%s, want: 0x%s",
				hex.EncodeToString([]byte{source[i]}),
				hex.EncodeToString([]byte{state[i]}),
				hex.EncodeToString([]byte{expected[i]}))
		}
...
func TestShiftRows(t *testing.T) {
	source := [][]byte{
		[]byte{0x53, 0xa4, 0xba, 0x01, 0x2f, 0x33, 0xab, 0xde, 0x85, 0x29, 0x99, 0x00, 0xff, 0xf0, 0x0f, 0xd4},
		[]byte{0x63, 0xca, 0xb7, 0x04, 0x09, 0x53, 0xd0, 0x51, 0xcd, 0x60, 0xe0, 0xe7, 0xba, 0x70, 0xe1, 0x8c},
	}
	expected := [][]byte{
		[]byte{0x53, 0x33, 0x99, 0xd4, 0x2f, 0x29, 0x0f, 0x01, 0x85, 0xf0, 0xba, 0xde, 0xff, 0xa4, 0xab, 0x00},
	...
	for i := range source {
		...
		for j := range state {
			if state[j] != expected[i][j] {
				t.Errorf("Invalid shiftrow %d for 0x%s, got: 0x%s, want: 0x%s",
					i,
...
func TestInvShiftRows(t *testing.T) {
	source := [][]byte{
		...
		[]byte{0x3e, 0x1c, 0x22, 0xc0, 0xb6, 0xfc, 0xbf, 0x76, 0x8d, 0xa8, 0x50, 0x67, 0xf6, 0x17, 0x04, 0x95},
	}
	expected := [][]byte{
		[]byte{0x54, 0x11, 0xf4, 0xb5, 0x6b, 0xd9, 0x70, 0x0e, 0x96, 0xa0, 0x90, 0x2f, 0xa1, 0xbb, 0x9a, 0xa1},
		[]byte{0x3e, 0x17, 0x50, 0x76, 0xb6, 0x1c, 0x04, 0x67, 0x8d, 0xfc, 0x22, 0x95, 0xf6, 0xa8, 0xbf, 0xc0},
	}
	for i := range source {
		state := make([]byte, len(source[i]))
		copy(state, source[i])
		...
		for j := range state {
			if state[j] != expected[i][j] {
				t.Errorf("Invalid invshiftrow %d for 0x%s, got: 0x%s, want: 0x%s",
					i,
					hex.EncodeToString([]byte{source[i][j]}),
					hex.EncodeToString([]byte{state[j]}),
					hex.EncodeToString([]byte{expected[i][j]}))
...
func TestMixColumns(t *testing.T) {
	...
	for i := 0; i < len(source); i++ {
		...
		for j := range state {
			if state[j] != expected[i][j] {
				t.Errorf("Invalid mixcolumns for 0x%s, got: 0x%s, want: 0x%s",
					hex.EncodeToString([]byte{source[i][j]}),
					hex.EncodeToString([]byte{state[j]}),
					hex.EncodeToString([]byte{expected[i][j]}))
			}
...
func TestInvMixColumns(t *testing.T) {
	...
	for i := 0; i < len(source); i++ {
		...
		for j := range state {
			if state[j] != expected[i][j] {
				t.Errorf("Invalid invmixcolumns for 0x%s, got: 0x%s, want: 0x%s",
					hex.EncodeToString([]byte{source[i][j]}),
					hex.EncodeToString([]byte{state[j]}),
					hex.EncodeToString([]byte{expected[i][j]}))
			}
...
func TestNewCipherValidKeyLengths(t *testing.T) {
	var err error
	var keyLengths []int = []int{16, 24, 32}
	var keys [][]byte = make([][]byte, len(keyLengths))
	for l := range keyLengths {
		keys[l] = make([]byte, keyLengths[l])
	}
...
func TestEncryption(t *testing.T) {
	...
	for i := range keys {
		cipher, err := NewCipher(keys[i])
		if err != nil {
			t.Errorf("Error creating new cipher for key %d: %s", i, err.Error())
			continue
		}
...
func TestDecryption(t *testing.T) {
	...
	for i := range keys {
		...
		res := make([]byte, len(expected[i]))
		cipher.Decrypt(inputs[i], res)
		for j := range res {
			if res[j] != expected[i][j] {
				t.Errorf(
					"Invalid decryption %d. Expected: 0x%s Got: 0x%s",
					i,
					hex.EncodeToString(expected[i]),
					hex.EncodeToString(res),
				)
